...
func dialTimeout(f newClientFunc, network, address string, opts ...*Option)	(client *Client, err error) {
	...
	case <-time.After(opt.ConnectTimeout):
		return nil, fmt.Errorf("rpc client: connect timeout: expect within %d", opt.MagicNumber)
	case result := <-ch:
		return result.client, result.err
	}
}
...
// eg, http@10.0.0.1:7001, tpc@10.0.0.1:9000, unix@/tmp/geerpc.sock
func XDial(rpcAddr string, opts ...*Option) (*Client, error) {
	parts := strings.Split(rpcAddr, "@")
	if len(parts) != 2 {
		return nil, fmt.Errorf("rpc cilent err: wrong format '%s', expect protocol@addr", rpcAddr)
	}
